...
export const btnPostShare = () => {
  ...
  btnPost.addEventListener("click", async (e) => {
    e.preventDefault();
    await insertData(post), wallArea.reset();
  });
};
  ...
  if ((window.location.hash = "#/timeline")) {
    onDataDocument((querySnapshot) => {
      ...
      querySnapshot.forEach((doc) => {
        ...
        html += `<div>
                    <div id = "user-photo-wall" class = "user-photo-wall">
                    <img class= "user-photo" id="user-photo" src="${
                      dataPost.photo
                    }"></img>
                    </div>
                        <p>${dataPost.name}</p>
                        ...
                        <div id = "btns-posts" class = "btns-posts">
                            <i class="like-post fa-solid fa-thumbs-up" data-id=${
                              doc.id
                            } id="like"></i>
                            <p class="like" >${dataPost.like?.length}</p>
                            ${showDeleteButtonOnlyIfOwnerUser(
                              dataPost.id,
                              doc.id
                            )}                            
                        </div>
